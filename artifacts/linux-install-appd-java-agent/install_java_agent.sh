# Script to install AppD Java agent on Azure DevTest lab Linux VMs via the artifact system.
#
# NOTE: Intended for use by the Azure DevTest Lab artifact system.
#
# Usage: 
#
# install_java_agent.sh --controller (CONTROLLER) --port (PORT) --application (APPLICATION) --tier (TIER) --account_name (ACCOUNT_NAME) --account_access_key (ACCOUNT_ACCESS_KEY)
#
